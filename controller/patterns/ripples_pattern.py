...
class RipplesPattern(PatternUV):
    ...
    async def animate(self, delta):
        self.time += delta
        # Remove old seeds
        self.ripple_seeds = list(filter(lambda s: self.time - s.time < 60.0, self.ripple_seeds))
        # Create new ones randomly
        if np.random.rand() < delta / self.params.period_s * self.params.speed:
        ...
        for segment in self.segments:
            ...
            segment.colors = scaleColors(self.params.color, intensities, self.params.amplitude_pct)
