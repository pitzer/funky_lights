...
class HexFile(object):
    ...
    def load(filename):
        ...
        for line in lines:
            ...
            if end_of_file:
                raise Exception(
                    "Record found after end of file on line %d" % lineno)
            ...
            if(computed_checksum != checksum):
                raise Exception(
                    "Record checksum doesn't match on line %d" % lineno)

            if record_type == 0:
                if byte_count == len(data):
                    current_address = (
                        address + extended_linear_address + extended_segment_address) & 0xffffffff
                    have_segment = False
                    ...
                    raise Exception(
                        "Data record reported size does not match actual size on line %d" % lineno)
            elif record_type == 1:
                end_of_file = True
            elif record_type == 2:
                if byte_count != 2 or len(data) != 2:
                    raise Exception(
                        "Byte count misreported in extended segment address record on line %d" % lineno)
                extended_segment_address = short(*data) << 4
            elif record_type == 3:
                if byte_count != 4 or len(data) != 4:
                    raise Exception(
                        "Byte count misreported in start segment address record on line %d" % lineno)
                cs = short(*data[0:2])
                ip = short(*data[2:4])
            elif record_type == 4:
                if byte_count != 2 or len(data) != 2:
                    raise Exception(
                        "Byte count misreported in extended linear address record on line %d" % lineno)
                extended_linear_address = short(*data) << 16
            elif record_type == 5:
                if byte_count != 4 or len(data) != 4:
                    raise Exception(
                        "Byte count misreported in start linear address record on line %d" % lineno)
                eip = long(*data)
            else:
    ...
    def pretty_string(self, stride=16):
        retval = []
        for segment in self.segments:
            retval.append('Segment @ 0x%08x (%d bytes)' %
...
class Segment(object):
    ...
    def pretty_string(self, stride=16):
        ...
        for r in ranges:
            retval.append('%08x ' %
                          r[0] + ' '.join(['%02x' % self[addr] for addr in r]))
        return '\n'.join(retval)
    ...
    def __getitem__(self, address):
        if isinstance(address, slice):
            if address.start not in self or address.stop-1 not in self:
                ...
                d = self.data[address.start-self.start_address:address.stop -
            ...
            if not address in self:
                raise IndexError(
                    "Address 0x%x is not in this segment" % address)
